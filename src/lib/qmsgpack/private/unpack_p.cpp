...
QVariant
MsgPackPrivate::unpack(const quint8 *p, const quint8 *end)
{
QVariantList d;
QVariant v;
...
quint8 *
MsgPackPrivate::unpack_type(QVariant &v, quint8 *p)
{
	if(*p <= 127)
...
quint8 *
MsgPackPrivate::unpack_nil(QVariant &v, quint8 *p)
{
    Q_UNUSED(p)
    Q_UNUSED(v)
    v = QVariant();
...
quint8 *
MsgPackPrivate::unpack_never_used(QVariant &v, quint8 *p)
{
    Q_UNUSED(p)
    Q_UNUSED(v)
    v = QVariant();
...
quint8 *
MsgPackPrivate::unpack_false(QVariant &v, quint8 *p)
{
    Q_UNUSED(p)
    v = false;
...
quint8 *
MsgPackPrivate::unpack_true(QVariant &v, quint8 *p)
{
    Q_UNUSED(p)
    v = true;
...
quint8 *
MsgPackPrivate::unpack_positive_fixint(QVariant &v, quint8 *p)
{
...
quint8 *
MsgPackPrivate::unpack_negative_fixint(QVariant &v, quint8 *p)
{
...
quint8 *
MsgPackPrivate::unpack_uint8(QVariant &v, quint8 *p)
{
    v = static_cast<quint32>(*(++p));
...
quint8 *
MsgPackPrivate::unpack_uint16(QVariant &v, quint8 *p)
{
    p++;
...
quint8 *
MsgPackPrivate::unpack_uint32(QVariant &v, quint8 *p)
{
    p++;
...
quint8 *
MsgPackPrivate::unpack_array32(QVariant &v, quint8 *p)
{
    p++;
...
quint8 *
MsgPackPrivate::unpack_map_len(QVariant &v, quint8 *p, quint32 len)
{
QVariantMap str_map;
QUIntMap uint_map;
QVariant key, val;
	...
	for(quint32 i = 0; i < len; ++i)
	{
		...
		if(key.type() == QVariant::String || str_map.size()) // if at least one key is a string then all keys in the map will be strings.
			str_map.insert(key.toString(), val); // key is string
		else
			uint_map.insert(key.toUInt(), val); // key is number
    }
//....................
	if(str_map.size()) // if string keys exist then map keys can only be string keys.
	{
		if(uint_map.size()) // if there are numeric keys then convert it keys to strings.
			for(const auto& k : uint_map.keys())
				str_map[QString::number(k)] =uint_map.value(k);

		v = str_map;
	}
	else
		v = QVariant::fromValue(uint_map);

return(p);
}

quint8 *
MsgPackPrivate::unpack_fixmap(QVariant &v, quint8 *p)
{
quint32 len = (*p++) & 0x0f; // 0b00001111 - total objects[key, value]
